// Humony 전용 VexFlow 렌더링 로직. Swift(VexFlowScoreView)가 window.renderScore(data)를
  ...
  for (var idx = 0; idx < n; idx++) {
    ...
    (function (capturedIndex) {
      rect.addEventListener('click', function () {
        ...
        if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.humonyNoteTap) {
          window.webkit.messageHandlers.humonyNoteTap.postMessage(capturedIndex);
        }
      });
    })(idx);
