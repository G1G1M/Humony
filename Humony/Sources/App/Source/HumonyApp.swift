...
struct HumonyApp: App {
    ...
    private static func makeContainer() -> ModelContainer {
        ...
        do {
            ...
            #if DEBUG
            print("[HumonyApp] 저장소를 열지 못했다(스키마 불일치로 추정) — 비우고 다시 만든다: \(error)")
            #endif
            removeStoreFiles()
        }
        ...
        do {
            ...
            #if DEBUG
            print("[HumonyApp] 저장소를 새로 만들지도 못했다 — 메모리 전용으로 실행한다: \(error)")
            #endif
