...
/// 재합성" 구조 개선의 1단계 — C++ 쪽(`HumonyWorldBridge.cpp`)의 불투명 포인터를
...
final class PitchShifterWorldAnalysis {
    ...
    init?(samples: [Float], sampleRate: Double, d4cThreshold: Double = 0.85) {
        ...
        guard let handle = input.withUnsafeBufferPointer({ buffer in
            HumonyWorldAnalyze(buffer.baseAddress, Int32(input.count), Int32(sampleRate), d4cThreshold)
        }) else { return nil }

        self.handle = handle
        self.f0Length = Int(HumonyWorldF0Length(handle))
        self.framePeriodMs = HumonyWorldFramePeriodMs(handle)
        self.inputLength = Int(HumonyWorldInputLength(handle))

        var f0 = [Double](repeating: 0, count: f0Length)
        f0.withUnsafeMutableBufferPointer { buffer in
            HumonyWorldGetF0(handle, buffer.baseAddress)
        }
        self.f0 = f0
    }

    deinit {
        HumonyWorldFreeAnalysis(handle)
    }
    ...
    func synthesize(f0: [Double], formantRatio: Double = 1.0) -> [Float] {
        ...
        f0.withUnsafeBufferPointer { f0Buffer in
            output.withUnsafeMutableBufferPointer { outputBuffer in
                HumonyWorldSynthesizeWithF0(handle, f0Buffer.baseAddress, formantRatio, outputBuffer.baseAddress)
            }
        }
