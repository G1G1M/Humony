#ifndef HumonyWorldBridge_h
#define HumonyWorldBridge_h
...
// handle 기반 API. 위 HumonyWorldPitchShift가 매번 새로 하던 분석(F0/스펙트럼 포락선/
...
// 끊기지 않는다. HumonyWorldPitchShift 자체도 내부적으로 이 API로 구현되어 있어(하위호환
// 유지), 기존 단일 호출부는 그대로 동작한다.
typedef struct HumonyWorldAnalysis HumonyWorldAnalysis;

// input을 한 번 분석해서 핸들을 만든다. 실패(길이가 분석 최소 길이보다 짧음 등)하면 NULL.
// 반환된 핸들은 반드시 HumonyWorldFreeAnalysis로 해제해야 한다.
//
...
// 분석된 F0 프레임 개수 — HumonyWorldGetF0로 꺼내거나 HumonyWorldSynthesizeWithF0에
...
// 분석에 쓰인 원본 오디오 길이(샘플 수) — HumonyWorldSynthesizeWithF0의 output 버퍼를
// 이 길이만큼 미리 할당해야 한다.
int HumonyWorldInputLength(const HumonyWorldAnalysis *analysis);

// 원본(수정 전) F0 곡선을 꺼낸다. outF0는 호출자가 HumonyWorldF0Length(analysis)만큼
// 미리 할당해둔 버퍼.
void HumonyWorldGetF0(const HumonyWorldAnalysis *analysis, double *outF0);

// modifiedF0(길이 = HumonyWorldF0Length(analysis))로 재합성한다 — 스펙트럼 포락선/
// 비주기성은 analysis에 저장된 원본 그대로 재사용하고(분석을 다시 하지 않음), F0만
// modifiedF0로 바꿔치기한다. formantRatio는 HumonyWorldPitchShift와 같은 의미
// (1.0이면 포락선 그대로, 그 외엔 워핑). output은 HumonyWorldInputLength(analysis)
