...
- [x] **로드맵 Phase 5 — 악보를 정통 오선보로 전면 교체 + Phase 6(표시/재생 공유 토글) 동시 완료**: 처음 시도한 "피치 하이웨이"(색깔 막대) 방식이 "악보 느낌이 안 난다"는 반려를 받아, `StaffGeometry`(좌표 계산, 신규)+`SheetMusicView`(그리기, 신규)로 정통 5선보를 새로 만들었다 — 다이어토닉 레터(옥타브당 7개) 단위로 세로 위치를 매기고(반음이 아니라), 흰건반 사이 음은 같은 자리에 샵(#) 표기, 5선 밖은 덧줄. 멜로디=높은음자리표/베이스+3도+5도=낮은음자리표(그랜드 스태프, SATB 합창 악보와 같은 이유). 박자는 표기하지 않고(템포 미검출) 시간축을 가로 위치에 그대로 씀. 노트헤드 옆에 작은 음이름 라벨은 유지(악보 못 읽어도 무슨 음인지 알 수 있게). 성부 on/off 토글이 "내 목소리로 화음" 재생과 **같은 `mutedVoices` 상태를 공유**하도록 설계해서, 별도 로드맵 항목이던 Phase 6이 이 구현 하나로 같이 끝남. `PitchHighwayView.swift`는 삭제. 유닛테스트 92개(84→92, `StaffGeometryTests` 8개 추가) 통과, arm64 컴파일 확인(기기 연결 해제 상태, 다음 연결 시 실기기 확인 필요)
...
| `StaffGeometry` | MIDI 노트 → 오선보 좌표(줄/칸, 샵 여부, 덧줄) 변환 |
