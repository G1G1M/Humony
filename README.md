...
- [ ] **"빠른 녹음 → 악보 → 연습" 전체 로드맵 진행 중 (Phase 1/9)**: `MelodySegmenter` 추가 — 녹음 전체를 배치로 분석해 음표(음높이+시작시간+길이)로 잘라내는 신규 컴포넌트(윈도우 분석 → 디바운스 → 런랭스 인코딩). 유닛테스트 7개(지속음/붙은 두 음/무음분리 두 음/짧은 blip 제거/비브라토 한 음으로 유지 등) 통과. UI 없는 순수 로직 단계라 실기기 확인은 다음 단계(빠른 녹음 UI)에서 진행
