...
- [x] **악보를 VexFlow(오픈소스 오선보 렌더링 JS 라이브러리, MIT)로 재교체**: 직접 구현한 오선보도 "너무 보기 힘들다"는 재반려를 받아, 사용자가 실제 아카펠라 악보 레퍼런스와 함께 VexFlow를 직접 제안 — `VexFlowScoreView`(신규, `WKWebView` 브릿지)가 로컬 HTML(벤더링한 `vexflow.js`, ~970KB, CDN 없이 완전 오프라인)을 로드하고 Swift가 JSON을 만들어 `renderScore()`를 호출. 레퍼런스처럼 성부마다 개별 오선(멜로디/5도/3도=높은음자리표, 베이스=낮은음자리표), 박자는 검출 불가라 4분음표 4개=1마디로 단순화(VexFlow의 자동 스페이싱으로도 출판 수준 균등 간격 확보), 음이름은 VexFlow의 가사(Annotation) API 자리에 표시, 배경은 항상 흰 종이 고정(앱 다크모드 무관). 성부 on/off는 그대로 `mutedVoices` 공유 유지. `SheetMusicView`/`StaffGeometry`(+테스트)는 완전 삭제. 유닛테스트 84개(92→84) 통과, 실기기(Ian) 빌드+설치 완료(잠금으로 자동 실행 대기) — "음도 안 맞는 것 같다"는 피드백은 원인 미확정, 다음 확인 필요
