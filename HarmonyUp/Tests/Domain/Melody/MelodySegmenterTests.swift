...
final class MelodySegmenterTests: XCTestCase {
    ...
    // MARK: - 흡수 결과의 대표 음높이 (149절, 실기기 로그 실측)

    /// 2026-08-24 실기기 로그 실측 — G장조 녹음인데 최종 음표에 `A#3`가 들어갔다.
    /// 흡수 전 후보는 `A3(0.09) A#3(0.12) A3(0.15)`로 **A3 합계가 0.24초, A#3는 0.12초**인데도
    /// 조각 하나하나로 보면 매번 이웃이 더 길어서 A#가 대표로 뽑혔다.
    ///
    /// 흡수는 "이 구간을 한 음으로 본다"는 판단이므로, 그 구간에서 **실제로 가장 오래 유지된
    /// 음높이**가 대표가 돼야 한다.
    func testAbsorbShortRunsPicksThePitchHeldLongestAcrossTheMergedRun() {
        let notes = [
            MelodySegmenter.SegmentedNote(midiNote: 55, onsetTime: 0.00, duration: 0.65, averageConfidence: 0.99), // G3
            MelodySegmenter.SegmentedNote(midiNote: 57, onsetTime: 0.65, duration: 0.09, averageConfidence: 0.97), // A3
            MelodySegmenter.SegmentedNote(midiNote: 58, onsetTime: 0.74, duration: 0.12, averageConfidence: 1.00), // A#3
            MelodySegmenter.SegmentedNote(midiNote: 57, onsetTime: 0.86, duration: 0.15, averageConfidence: 0.99), // A3
            MelodySegmenter.SegmentedNote(midiNote: 59, onsetTime: 1.01, duration: 0.29, averageConfidence: 0.99)  // B3
        ]

        let result = MelodySegmenter.absorbShortRuns(notes, minimumDuration: 0.18)

        XCTAssertEqual(result.map(\.midiNote), [55, 57, 59], "합쳐진 구간에서 더 오래 유지된 A3가 대표여야 한다")
        // 흡수된 구간 전체(0.65~1.01)가 한 음으로 남는다 — 시간이 사라지면 안 된다.
        XCTAssertEqual(result[1].onsetTime, 0.65, accuracy: 0.001)
        XCTAssertEqual(result[1].duration, 0.36, accuracy: 0.001)
    }

    /// **회귀** — 진짜 포르타멘토는 118·119절에 확정한 동작 그대로여야 한다. 긴 음에 짧은
    /// 슬라이드 하나가 붙는 경우는 다수결로도 답이 같다(긴 쪽이 압도적).
    func testAbsorbShortRunsStillKeepsTheLongNoteForRealPortamento() {
        let notes = [
            MelodySegmenter.SegmentedNote(midiNote: 62, onsetTime: 0.00, duration: 0.43, averageConfidence: 0.9), // D3
            MelodySegmenter.SegmentedNote(midiNote: 63, onsetTime: 0.43, duration: 0.09, averageConfidence: 0.5)  // D#3 슬라이드
        ]

        let result = MelodySegmenter.absorbShortRuns(notes, minimumDuration: 0.18)

        XCTAssertEqual(result.map(\.midiNote), [62])
        XCTAssertEqual(result[0].duration, 0.52, accuracy: 0.001)
    }

    /// 동점이면 흡수한 쪽(기존 동작)을 유지한다 — 불필요한 변화를 만들지 않기 위한 규칙.
    func testAbsorbShortRunsKeepsAbsorbingPitchOnDurationTie() {
        let notes = [
            MelodySegmenter.SegmentedNote(midiNote: 60, onsetTime: 0.00, duration: 0.30, averageConfidence: 0.9), // C4
            MelodySegmenter.SegmentedNote(midiNote: 61, onsetTime: 0.30, duration: 0.10, averageConfidence: 0.5), // C#4
            MelodySegmenter.SegmentedNote(midiNote: 60, onsetTime: 0.40, duration: 0.10, averageConfidence: 0.9)  // C4
        ]

        let result = MelodySegmenter.absorbShortRuns(notes, minimumDuration: 0.18)

        XCTAssertEqual(result.map(\.midiNote), [60], "C4가 더 길므로 대표는 C4")
    }
