...
/// `PitchShifterWSOLA`(2026-08-20에 `c94288b^`에서 복원)의 테스트 — 당시 PSOLA로
/// 교체되기 직전 최종 다듬어진 버전의 테스트를 그대로 가져왔다(파일명만 조정).
final class PitchShifterWSOLATests: XCTestCase {
    ...
    // PitchShifterWSOLA를 검증하는 가장 확실한 방법은 이미 만들어둔 YINPitchDetector로
    ...
    func testShiftUpByMajorThirdRaisesDetectedPitch() throws {
        ...
        let shifted = PitchShifterWSOLA.shift(samples: input, pitchRatio: ratio, sampleRate: sampleRate)
    ...
    func testTimeStretchAlonePreservesPitchAtNonUnityFactor() throws {
        let input = sineWave(frequency: 440.0, sampleCount: 8192)
        let stretched = PitchShifterWSOLA.timeStretch(samples: input, factor: 1.26)
    ...
    func testResampleAloneShiftsPitchByRate() throws {
        let input = sineWave(frequency: 440.0, sampleCount: 8192)
        let resampled = PitchShifterWSOLA.resample(samples: input, rate: 1.26)
    ...
    func testIdentityRatioPreservesPitch() throws {
        let input = sineWave(frequency: 440.0, sampleCount: 8192)
        let shifted = PitchShifterWSOLA.shift(samples: input, pitchRatio: 1.0, sampleRate: sampleRate)
        let middle = middleSegment(of: shifted, length: 4096)
        let candidates = YINPitchDetector.detectPitch(samples: middle, sampleRate: sampleRate)
        let detected = try XCTUnwrap(candidates.first)
        XCTAssertEqual(detected.frequency, 440.0, accuracy: 20)
    ...
    func testOutputLengthApproximatelyMatchesInput() {
        let input = sineWave(frequency: 440.0, sampleCount: 8192)
        let shifted = PitchShifterWSOLA.shift(samples: input, pitchRatio: 1.26, sampleRate: sampleRate)

        XCTAssertEqual(Double(shifted.count), Double(input.count), accuracy: 300)
    }

    func testEmptyInputReturnsEmpty() {
        XCTAssertTrue(PitchShifterWSOLA.shift(samples: [], pitchRatio: 1.5, sampleRate: sampleRate).isEmpty)
    }
    ...
    func testAntiAliasFilterAttenuatesHighFrequencyMuchMoreThanLowFrequency() {
        ...
        let filteredHigh = PitchShifterWSOLA.antiAliasFilter(highFrequency, decimationRate: 4.0)
        let filteredLow = PitchShifterWSOLA.antiAliasFilter(lowFrequency, decimationRate: 4.0)
    ...
    func testAntiAliasFilterSkipsWhenNotDownsampling() {
        ...
        XCTAssertEqual(PitchShifterWSOLA.antiAliasFilter(samples, decimationRate: 1.0), samples)
        XCTAssertEqual(PitchShifterWSOLA.antiAliasFilter(samples, decimationRate: 0.5), samples)
    }
    ...
    func testShiftWithExpectedFrequencyStillShiftsPitchCorrectly() throws {
        ...
        let shifted = PitchShifterWSOLA.shift(samples: input, pitchRatio: ratio, sampleRate: sampleRate, expectedFrequency: 220.0)
