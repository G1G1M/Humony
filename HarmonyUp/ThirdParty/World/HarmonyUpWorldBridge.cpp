...
// 129절 — HarmonyUpWorldPitchShift가 예전에 한 함수 안에서 순서대로 하던 "분석(F0/스펙트럼
...
HarmonyUpWorldAnalysis *HarmonyUpWorldAnalyze(const double *input, int length, int sampleRate) {
  ...
  auto *analysis = new HarmonyUpWorldAnalysis();
...
void HarmonyUpWorldPitchShift(const double *input, int length, int sampleRate,
  ...
  HarmonyUpWorldSynthesizeWithF0(analysis, shiftedF0.data(), formantRatio, output);
  HarmonyUpWorldFreeAnalysis(analysis);
}
