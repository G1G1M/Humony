...
enum ChordGenerator {
    ...
    enum Interval: Hashable, CaseIterable {
        ...
        /// `formantRatio`로 그대로 전달된다(Phase 8 Task 1, docs/CONCEPTS.md 77절). 원래는
        /// 베이스를 아래로(성도가 긴 실제 저음처럼), 3도/5도는 위로(맑은 두성처럼) 옮겨 다른
        /// 성역의 사람이 부르는 느낌을 내려던 것이었다.
        ///
        /// **95절 이후 1.0(끔)으로 되돌림**: 실기기 청취에서 "화음이 기계음처럼 들리고
        /// 멜로디보다 느리게 들려서 싱크가 안 맞는다", "다듬기 전이 오히려 자연스러웠다"는
        /// 피드백을 받음 — WORLD가 F0/포먼트/비주기성을 분석해서 다시 합성하는 보코더 방식
        /// 자체가 트랜지언트(공격 순간)를 뭉갤 수 있는데, 포먼트까지 크게(0.85/1.15) 틀면
        /// 그 재합성 아티팩트가 커진다는 게 유력한 원인 — 사용자가 "포먼트 워핑부터 끄기"를
        /// 선택. 피치는 여전히 WORLD로 옮기지만(포먼트는 원래 성도 그대로, 1.0) 재합성
        /// 강도가 줄어서 더 자연스럽게 들릴 것으로 기대. 성부별 "다른 사람이 부르는" 개성은
        /// 약해지지만, 이후 청취 결과에 따라 다시 켜거나 값을 조정할 수 있다(구조는 그대로 남김).
