...
///
/// **v3(48절)**: 직접 구현한 PSOLA(v2, 47절)를 WORLD(BSD 라이선스, 음성 분석/합성계에서
/// 널리 검증된 오픈소스 라이브러리)로 교체했다. 직접 구현이 합성 신호(순음/배음 신호)와
/// YIN 재검출로는 "숫자상 맞는" 결과를 냈지만, 실제 목소리로는 여전히 부자연스럽게
/// 들린다는 피드백을 받았다 — 소규모 팀이 몇 시간 안에 재현하기엔 음성 분석/합성이
/// 원래 그 자체로 하나의 연구 분야다. WORLD는 F0(기본주파수)와 스펙트럼 포락선(포먼트)/
/// 비주기성(숨소리 성분)을 애초에 분리해서 분석하므로, F0만 옮기고 나머지는 그대로
/// 재합성하면 포먼트가 확실히 보존된다. 실제 호출부는
