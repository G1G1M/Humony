...
///
/// **123절, 화음 재설계 2단계에서 git 히스토리(`c94288b^`, PSOLA로 교체되기 직전 최종 WSOLA
/// 버전)에서 그대로 복원**: 93~115절 20라운드 시행착오에서 WSOLA→PSOLA→WORLD로 세 번
/// 갈아탔지만, 121절에서 그 불만의 상당 부분(끊김/타이밍)이 실은 재생 크로스페이드 버그였음이
/// 밝혀졌다 — 그래서 가장 단순했던 WSOLA부터 그 위에서 다시 시작한다(변수 격리).
enum PitchShifter {
